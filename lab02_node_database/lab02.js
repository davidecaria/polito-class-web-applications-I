...
function FilmLibrary() {
  ...
  this.closeDatabase = () => {
    try {
        ...
        console.log("It is not possible to close the db");
    }
  }
      ...
      return new Promise((resolve, reject) => {
        ...
        db.all(query,[],(err,rows) => {
            if(err){
                ...
                const films = rows.map( record => new Film(record.id,record.title, record.favorite == 1, record.rating));
                resolve(films);
            }
        })
    })
  ...
  this.getFavourites = () => {
    return new Promise((resolve,reject) => {
        const query = 'SELECT * FROM FILMS WHERE favourite = True';
        db.all(query,[],(err,rows) => {
            if(err){
                reject(err);
            }else{
  ...
  this.getWatchedToday = () => {
    ...
    this.getWatchedBefore = (dateToBeChecked) =>{
        return new Promise((resolve,reject) => {
            const query = 'SELECT * FROM FILMS WHERE watchdate = ?';
            db.all(query,[dateToBeChecked.format('YYYY-MM-DD')],(err,rows) => {
                if(err){
                    reject(err);
                }else{
                    const films = rows.map( record => new Film(record.id,record.title, record.favorite == 1, record.rating));
    ...
    this.getTitle = (word) => {
        return new Promise((resolve,reject) => {
            const query = 'INSERT INTO films(id,title,favourite,watchdate,rating) VALUES(?,?,?,?,?)';
            const param = [film.id,film.title,film.isFavorite,film.watchDate.format('YYYY-MM-DD'),film.rating];
